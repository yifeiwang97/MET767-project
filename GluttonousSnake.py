...
class GameState:
    ...
    def frame_step(self,input_actions):
        ...
        # 检测输入正确性
        if input_actions == 0 or input_actions == 1 or input_actions == 2 or input_actions == 3:  # 检查输入正常
            if input_actions == 0  and self.pos != (0,1):
                # 这个判断是为了防止蛇向上移时按了向下键，导致直接 GAME OVER
                self.pos = (0, -1)

            elif input_actions == 1 and self.pos != (0,-1):
                #向下
                self.pos = (0, 1)
            elif input_actions == 2 and self.pos != (1,0):
                #向左
                self.pos = (-1, 0)
            elif input_actions == 3 and self.pos != (-1,0):
                #向右
                self.pos = (1, 0)
            else:
                pass
